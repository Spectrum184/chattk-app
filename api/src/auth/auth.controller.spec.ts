...
describe("AuthController", () => {
    ...
    it("should return the user", () => {
        expect(authController.user(req as any)).toStrictEqual({
            id: req.user.id,
    ...
    it("should logout the user", async () => {
        jest.spyOn(authService, "deleteSession").mockResolvedValue({
            acknowledged: true,
            deletedCount: 1,
        });

        await expect(authController.logout(req as any)).resolves.toBe(
            undefined
        );
        expect(authService.deleteSession).toBeCalledWith(req.user.sessionId);
        expect(websocketService.logoutSession).toBeCalledWith(
            req.user.sessionId
        );
    ...
    it("should create session (login)", async () => {
        ...
        expect(authService.createSession).toBeCalledWith(
            req.user.id,
