...
describe("UsersController", () => {
    ...
    describe("addFriend", () => {
        describe("Send friend request", () => {
            ...
            afterEach(() => {
                expect(websocketService.emitNewFriendRequest).toBeCalledWith(
                    { id: req.user.id, username: req.user.username },
                    addFriendResult.user
                );
                expect(websocketService.emitFriendAdded).not.toBeCalled();
            });

            it("should send a friend request by id", async () => {
                jest.spyOn(usersService, "addFriend").mockResolvedValue(
                    addFriendResult
                );
                ...
                expect(usersService.addFriend).toBeCalledWith(
                    addFriendResult.user.id,
                    req.user,
                    true
                );
            });
        ...
        describe("accept a friend request", () => {
            ...
            afterEach(() => {
                expect(websocketService.emitFriendAdded).toBeCalledWith(
                    req.user.id,
                    addFriendResult.user.id,
                    addFriendResult.chat
                );
    ...
    describe("removeFriend", () => {
        it("should remove a friend", async () => {
            ...
            await expect(
                usersController.removeFriend(result.user.id, req as any)
            ).resolves.toStrictEqual(result);
            expect(usersService.removeFriend).toBeCalledWith(
                result.user.id,
                req.user
            );
            expect(websocketService.emitFriendRemoved).toBeCalledWith(
                req.user.id,
                result.user.id,
                result.message,
                result.chatId
