...
describe("UsersService", () => {
    ...
    describe("removeFriend", () => {
        ...
        describe("happy path", () => {
            afterEach(() => {
                expect(usersService.findOneById).toBeCalledWith(
                    userNoProfile.id
                );
                ...
                expect(mongo.users.updateOne).toBeCalledWith(
                    ...
                    {
                        $pull: {
                            "profile.relations": { id: userNoProfile.id },
                        },
                    },
                    { session: expect.any(Object) }
                );
            ...
            const friendRequestCanceledOrDeclied = async (
                canceled?: boolean
            ) => {
    ...
    describe("addFriend", () => {
        ...
        describe("happy path", () => {
            describe("send a friend request", () => {
                ...
                it("should send by username", async () => {
                    ...
                    await expect(
                        usersService.addFriend(receiver.username, sender, false)
                    ).resolves.toStrictEqual(result);

                    expect(usersService.findOneByName).toBeCalledWith(
                        receiver.username
                    );
                ...
                it("should send by id", async () => {
                    ...
                    await expect(
                        usersService.addFriend(receiver.id, sender, true)
                    ).resolves.toStrictEqual(result);
            ...
            describe("accept a friend request", () => {
                ...
                describe("has existing chat", () => {
                    ...
                    it("should accept by id", async () => {
                        ...
                        await expect(
                            usersService.addFriend(senderUser.id, user, true)
                        ).resolves.toStrictEqual(result);
        ...
        describe("unhappy path", () => {
            it("should throw NotFoundException when user id is not found", async () => {
                ...
                await expect(
                    usersService.addFriend(sender.id, receiver, true)
            ...
            it("should throw NotFoundException when username is not found", async () => {
                ...
                await expect(
                    usersService.addFriend(sender.username, receiver, false)
                ).rejects.toThrow(NotFoundException);
                expect(usersService.findOneByName).toBeCalledWith(
                    sender.username
                );
            ...
            it("should throw ConflictException when user sends a request to themself", async () => {
                ...
                const fn = () =>
                    usersService.addFriend(sender.username, sender, false);
            ...
            it("should throw ConflictException when user is already a friend", async () => {
                const otherUser: User = {
                    ...
                    profile: {
                        relations: [
                            { id: sender.id, status: RelationStatus.Friend },
                        ],
                    },
                };
                jest.spyOn(usersService, "findOneByName").mockResolvedValue(
                ...
                const fn = () =>
                    usersService.addFriend(otherUser.username, sender, false);
            ...
            it("should throw ConflictException when user already sent a request", async () => {
                const otherUser: User = {
                    ...
                    profile: {
                        relations: [
                            { id: sender.id, status: RelationStatus.Incoming },
                        ],
                    },
                };
                jest.spyOn(usersService, "findOneByName").mockResolvedValue(
            ...
            it("should throw ConflictException when user is blocked by otherUser", async () => {
                const otherUser: User = {
                    ...
                    profile: {
                        relations: [
                            { id: sender.id, status: RelationStatus.Blocked },
                        ],
                    },
                };
                jest.spyOn(usersService, "findOneByName").mockResolvedValue(
    ...
    describe("createUser", () => {
        ...
        it("should create user", async () => {
            ...
            expect(mongo.users.insertOne).toBeCalledWith({
                _id: userId,
                username,
                passwordHash,
            });
