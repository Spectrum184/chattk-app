...
describe("WebsocketGateway", () => {
    ...
    describe("handleConnection", () => {
        ...
        describe("happy path", () => {
            ...
            it("should successfully handle connection (single active session)", async () => {
                const mockedMap = new Map([
                    ["id1", { sIds: ["socketsess"], online: true }],
                ]);
                websocketGateway.sockets = mockedMap;

                jest.spyOn(
                    websocketService,
                    "authenticateUserFromSocket"
                ).mockResolvedValue({ sessionId, ...data });

                await expect(
                    websocketGateway.handleConnection(socket as any)
                ).resolves.not.toBeDefined();

                expect(
                    websocketService.authenticateUserFromSocket
                ).toBeCalledWith(socket, mockedMap);
                expect(mockedMap.get(data.id)).toEqual({
                    sIds: [socket.id],
                    online: true,
                });
                expect(websocketService.emitUserOnline).toBeCalledWith(
                    data.id,
                    [data.users[0].id],
                    true
                );
            ...
            it("should successfully handle connection (no friends)", async () => {
                ...
                dataNoFriends.users = data.users.map(
                    ({ relationship, ...rest }) => ({
                        ...rest,
                        relationship: undefined,
                    })
                ...
                jest.spyOn(
                    websocketService,
                    "authenticateUserFromSocket"
                ).mockResolvedValue({ sessionId, ...dataNoFriends });
        ...
        describe("unhappy path", () => {
            ...
            it("should emit and disconnect socket on Unauthorized error", async () => {
                ...
                expect(websocketService.emitException).toBeCalledWith(
                    socket,
                    "onGatewayConnection",
                    "Invalid session."
                );
    ...
    describe("handleDisconnect", () => {
        ...
        it("should remove socket session from state and notify friends (single active session)", async () => {
            const mockedMap = new Map([
                [socket.user.id, { sIds: [socket.id], online: true }],
            ]);
            ...
            jest.spyOn(
                websocketService,
                "getFriendIdsFromSocket"
            ).mockResolvedValue(["id1"]);

            await expect(
                websocketGateway.handleDisconnect(socket as any)
            ).resolves.not.toBeDefined();
            expect(mockedMap.get(socket.user.id)).toEqual({
                sIds: [],
                online: disconnectTime,
            });
            expect(websocketService.getFriendIdsFromSocket).toBeCalledWith(
                socket
            );
            expect(websocketService.emitUserOnline).toBeCalledWith(
                socket.user.id,
        ...
        it("should remove socket session from state and notify friends (multiple active session)", async () => {
            const mockedMap = new Map([
                [
                    socket.user.id,
                    { sIds: [socket.id, "othersess"], online: true },
                ],
            ]);
            const disconnectTime = new Date();
        ...
        it("should do nothing if client isn't authenticated", async () => {
            await expect(
                websocketGateway.handleDisconnect({} as any)
            ).resolves.not.toBeDefined();
