...
describe("WebsocketService", () => {
    ...
    describe("getUserFromSocket", () => {
        ...
        it("should authenticate user from socket auth object and return user", async () => {
            ...
            await expect(
                websocketService.getUserFromSocket(socket as any)
            ).resolves.toStrictEqual({
                sessionId,
                user,
            });
            expect(authService.validateToken).toBeCalledWith(
    ...
    describe("authenticateUserFromSocket", () => {
        ...
        it("should return relevant data for the 'Ready' socket event", async () => {
            ...
            const onlineSockets = new Map([
                [user.user.profile.relations[0].id, true],
                [user.user.profile.relations[0].id, lastOnlineDateOiw as any],
            ]);
            ...
            const relatedUsers = [
                {
                    ...
                    username: "user1",
                    online: onlineSockets.get(
                        user.user.profile.relations[0].id
                    ),
                ...
                },
                {
                    ...
                    username: "user2",
                    online: onlineSockets.get(
                        user.user.profile.relations[1].id
                    ),
            ...
            jest.spyOn(chatService, "getMessagesById").mockResolvedValue(
                lastMessages
            );
            jest.spyOn(websocketService, "getUserFromSocket").mockResolvedValue(
                user
            );
            jest.spyOn(
                usersService,
                "findRelatedUsersWithStatus"
            ).mockResolvedValue(relatedUsers);

            await expect(
                websocketService.authenticateUserFromSocket(
                    socket as any,
                    onlineSockets
                )
            ).resolves.toStrictEqual({
            ...
            expect(usersService.findRelatedUsersWithStatus).toBeCalledWith(
                relatedUsers.map((usr) => usr.id),
                onlineSockets,
                user.user.profile.relations
            );
    ...
    describe("getFriendIdsFromSocket", () => {
        it("should call getFriendIds", async () => {
            ...
            expect(
                websocketService.getFriendIdsFromSocket(socket as any)
    ...
    describe("emitFriendAdded", () => {
        it("should emit and join users to their chat room", () => {
            ...
            jest.spyOn(websocketService, "userRoom").mockImplementation(
                (id) => `prefix:${id}`
            );
            ...
            expect(
                websocketService.emitFriendAdded("user1", "user2", chat)
            ).not.toBeDefined();
            expect(websocketService.emitUpdateUser).toBeCalledWith(
                `prefix:user2`,
                {
                    id: "user1",
                    online: true,
            ...
            expect(websocketService.emitNewDirectChatJoin).toBeCalledWith(
                ["user1", "user2"],
                chat
            );
    ...
    describe("emitNewFriendRequest", () => {
        it("should emit new friend request", () => {
            ...
            expect(
                websocketService.emitNewFriendRequest(user1, user2)
            ).not.toBeDefined();
    ...
    describe("emitFriendRemoved", () => {
        ...
        it("should emit and not leave chat room", () => {
            ...
            expect(
                websocketService.emitFriendRemoved(
                    "user1",
                    "user2",
                    msg,
                    "chatId"
        ...
        it("should emit and leave chat room", () => {
            ...
            runCommonTests(msg);
            expect(websocketService.leaveDirectChatRoom).toBeCalledWith(
                ["user1", "user2"],
                "chatId"
            );
        });
    ...
    describe("emitUserOnline", () => {
        it("should emit user online", () => {
            ...
            expect(
                websocketService.emitUserOnline("user1", recipients, true)
            ).not.toBeDefined();
            expect(websocketService.emitUpdateUser).toBeCalledWith(
                ["prefix:id1", "prefix:id2"],
                { id: "user1", online: true }
            );
        });
    });
    ...
    describe("emitUpdateUser", () => {
        it("should emit updated user data", () => {
            expect(
                websocketService.emitUpdateUser(
                    "room",
                    { data: "data" },
                    "a message"
                )
            ...
            expect(mockWebsocketEmit).toBeCalledWith("User:Update", {
                user: { data: "data" },
                message: "a message",
            });
        });
        ...
        it("should throw InternalServerError if no room is passed", () => {
            expect(() => websocketService.emitUpdateUser("", {})).toThrowError(
                InternalServerErrorException
            );
    ...
    describe("emitUpdateChat", () => {
        it("should emit updated chat data", () => {
            ...
            expect(
                websocketService.emitUpdateChat("room", chat as any)
            ).not.toBeDefined();
            expect(websocketGateway.server.to).toBeCalledWith("room");
        ...
        it("should throw InternalServerError if no room is passed", () => {
            expect(() =>
                websocketService.emitUpdateChat("", {} as any)
    ...
    describe("emitNewMessage", () => {
        it("should emit new message", () => {
            ...
            expect(
                websocketService.emitNewMessage("room", message as any, "ackId")
            ).not.toBeDefined();
            expect(websocketGateway.server.to).toBeCalledWith("room");
            expect(mockWebsocketEmit).toBeCalledWith("Message:New", {
                ...message,
                ackId: "ackId",
            });
        });
        ...
        it("should throw InternalServerError if no room is passed", () => {
            expect(() =>
                websocketService.emitNewMessage("", {} as any)
            ).toThrowError(InternalServerErrorException);
            expect(websocketGateway.server.to).not.toBeCalled();
            expect(mockWebsocketEmit).not.toBeCalled();
        });
    ...
    describe("emitException", () => {
        it("should emit exception event to the socket", () => {
            ...
            expect(
                websocketService.emitException(socket as any, type, message)
            ).not.toBeDefined();
    ...
    describe("emitNewDirectChatJoin", () => {
        ...
        it("should join users to a direct chat room and emit their updated chat", () => {
            ...
            jest.spyOn(websocketService, "directChatRoom").mockImplementation(
                (id) => `prefix:${id}`
            );
            expect(
                websocketService.emitNewDirectChatJoin(userIds, chat as any)
            ).not.toBeDefined();
            expect(websocketService.joinDirectChatRoom).toBeCalledWith(
                userIds,
                chat.id
            );
            expect(websocketService.emitUpdateChat).toBeCalledWith(
                `prefix:${chat.id}`,
                chat
            );
        });
    ...
    describe("joinDirectChatRoom", () => {
        it("should join users to a direct chat room", () => {
            ...
            expect(
                websocketService.joinDirectChatRoom(["user1", "user2"], chatId)
            ).not.toBeDefined();
            expect(websocketGateway.server.in).toBeCalledWith([
                `prefix:user1`,
                `prefix:user2`,
            ]);
    ...
    describe("leaveDirectChatRoom", () => {
        it("should remove users from a direct chat room", () => {
            ...
            expect(
                websocketService.leaveDirectChatRoom(["user1", "user2"], chatId)
            ).not.toBeDefined();
            expect(websocketGateway.server.in).toBeCalledWith([
                `prefix:user1`,
                `prefix:user2`,
    ...
    describe("userOnline", () => {
        it("should return true", () => {
            jest.spyOn(websocketGateway.sockets, "get").mockReturnValue({
                online: true,
            } as any);
    ...
    describe("logoutSession", () => {
        it("should disconnect sockets of a specific session id (not the socket session id)", () => {
            jest.spyOn(websocketService, "userSessRoom").mockImplementation(
                (id) => `prefix:${id}`
            );
            expect(
                websocketService.logoutSession("sessionId")
            ).not.toBeDefined();
            expect(websocketGateway.server.to).toBeCalledWith(
                `prefix:sessionId`
            );
        ...
        it("should throw InternalServerError if no session id is provided", () => {
            expect(() => websocketService.logoutSession("")).toThrowError(
                InternalServerErrorException
            );
            expect(websocketGateway.server.to).not.toBeCalled();
    ...
    describe("directChatRoom", () => {
        it("should return the room name suffixed with the chatId", () => {
            expect(websocketService.directChatRoom("chatId")).toBe(
                "chat-direct:chatId"
            );
        });
    });
    ...
    describe("userSessRoom", () => {
        it("should return the room name suffixed with the sessionId", () => {
            expect(websocketService.userSessRoom("sessionId")).toBe(
