...
export class WebsocketService implements OnModuleInit {
    ...
    emitBeginTyping(userId: string, chatId: string) {
        this.websocketGateway.server
            .to(this.directChatRoom(chatId))
            .except(this.userRoom(userId))
            .emit("Chat:BeginTyping", { chatId, userId });
    }
    ...
    emitEndTyping(userId: string, chatId: string) {
        this.websocketGateway.server
            .to(this.directChatRoom(chatId))
            .except(this.userRoom(userId))
            .emit("Chat:EndTyping", { chatId, userId });
    }
    emitUpdateUser(
        ...
        if (!emitTo || emitTo.length == 0) {
            this.logger.error({
                event: "ws_invalid_room,empty",
                msg: `emitUpdateUser received empty room string/array. This is likely unintentional as it will send the event to every socket.`,
            });
    ...
    emitUpdateChat(emitTo: string | string[], chat: Chat) {
        if (!emitTo || emitTo.length == 0) {
            this.logger.error({
                event: "ws_invalid_room,empty",
                msg: `emitUpdateChat received empty room string/array. This is likely unintentional as it will send the event to every socket.`,
            });
            throw new InternalServerErrorException();
        }
    ...
    emitNewMessage(
        ...
        if (!emitTo || emitTo.length == 0) {
            this.logger.error({
                event: "ws_invalid_room,empty",
                msg: `emitNewMessage received empty room string/array. This is likely unintentional as it will send the event to every socket.`,
            });
            throw new InternalServerErrorException();
        }
    ...
    logoutSession(sid: string) {
        if (!sid || !sid.trim().length) {
            this.logger.error({
                event: "ws_invalid_room,empty",
                msg: `logoutSession received empty room string. This is absolutely unintentional.`,
            });
            throw new InternalServerErrorException();
        }
        this.websocketGateway.server
